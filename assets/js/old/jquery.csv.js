    ...
    $.csv = {
            ...
            parse: function (csv, options) {
                ...
                function endOfEntry() {
                    ...
                    if (options.onParseEntry === undefined) {
                        ...
                        const hookVal = options.onParseEntry(
                            entry,
                            options.state
                        ); // onParseEntry Hook
            ...
            for (let i = 0; i < arrays.length; i++) {
                ...
                for (let j = 0; j < line.length; j++) {
                    ...
                    if (strValue.search(escMatcher) > -1) {
                        strValue =
                            config.delimiter + strValue + config.delimiter;
                    }
        ...
        fromObjects: function (objects, options, callback) {
            ...
            config.headers =
                "headers" in options ? options.headers : $.csv.defaults.headers;
            config.sortOrder =
                "sortOrder" in options ? options.sortOrder : "declare";
            config.manualOrder =
                "manualOrder" in options ? options.manualOrder : [];
            ...
            if (config.transform !== undefined) {
                ...
                for (let i = 0; i < origObjects.length; i++) {
                    objects.push(
                        config.transform.call(undefined, origObjects[i])
                    );
                }
            }
